...
{
    public class Lzss : IDecompressor, ICompressor
    {
        ...
        public int Compress(Stream input, Stream output)
        {
            ...
            do
            {
                ...
                if (_matchLength <= THRESHOLD) /* Not long enough match. Send one byte. */
                {
                    _matchLength = 1;
                    code_buf[0] |= mask;  /* 'Send one byte' flag */
                    code_buf[code_buf_ptr++] = _textBuffer[r];  /* Send uncoded. */
                }
                else /* Compression can be used */
                {
                    code_buf[code_buf_ptr++] = (byte)_matchPosition;
                    code_buf[code_buf_ptr++] = (byte)(((_matchPosition >> 4) & 0xF0) | (_matchLength - (THRESHOLD + 1))); /* Send position and length pair. Note match_length > THRESHOLD. */
                }
                ...
                last_match_length = _matchLength;
        ...
        private void InsertNode(int r)
        {
            ...
            for (; ; )
            {
                ...
                for (i = 1; i < F; i++)
                    if ((cmp = _textBuffer[key + i] - _textBuffer[p + i]) != 0)
                        break;
